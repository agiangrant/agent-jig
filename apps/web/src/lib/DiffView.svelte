...
import { createScrollGroup } from "./syncScroll.ts";

// One group per diff: its panes (split sides, stacked hunks) scroll in unison.
const scroll = createScrollGroup();
  ...
  <div class="diff">
    {#each hunks as h, i (i)}
      {#if diffMode.mode === "split"}
        <div class="split">
          <div class="pane del">
            {#if h.old.length}<Code code={h.old.join("\n")} path={filePath} numbered={diffMode.lineNumbers} startLine={h.startLine} scrollGroup={scroll} />{:else}<div class="empty">—</div>{/if}
          </div>
          <div class="pane add">
      ...
      {:else if diffMode.mode === "unified"}
        {#if h.old.length}<div class="block del"><Code code={h.old.join("\n")} path={filePath} numbered={diffMode.lineNumbers} startLine={h.startLine} scrollGroup={scroll} /></div>{/if}
      ...
      {:else}
        ...
        <div class="block {diffMode.side === 'before' ? 'del' : 'add'}">
          <Code code={side.join("\n")} path={filePath} numbered={diffMode.lineNumbers} startLine={h.startLine} scrollGroup={scroll} />
        </div>
      {/if}
    {/each}
  </div>
